  ...
  try {
    ...
    if (conteudo && conteudo.publicado_por !== req.user!.userId) {
      await pool.query(
        ...
        [conteudo.publicado_por, pedidoId,
...
export async function denunciarComentario(req: Request, res: Response): Promise<void> {
  try {
    ...
    const mensagem =
      `${comentario['autor_nome']} em "${comentario['conteudo_titulo']}": "${previaComentario}${previaComentario.length === 120 ? '…' : ''}"`
    for (const admin of admins as RowDataPacket[]) {
      await pool.query(
        `INSERT INTO notificacao (usuario_id, tipo, entidade_id, titulo, mensagem, link_destino)
         VALUES (?, 'comentario_denunciado', ?, 'Comentário denunciado', ?, '/admin')`,
        [admin['id'], comentarioId, mensagem],
      )
    }
