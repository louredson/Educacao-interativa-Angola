  ...
  if (email_destino) {
    ...
    if (destinatario && destinatario.id !== userId) {
      await pool.query(
        ...
        [
          destinatario.id,
          tipo === 'sala' ? 'convite_sala' : 'convite_topico',
          entidade_id,
          tipo === 'sala' ? 'Convite para uma sala de discussão' : 'Convite para um tópico privado',
          `Foste convidado(a) para ${tipo === 'sala' ? 'a sala' : 'o tópico'} "${nomeEntidade}".`,
          `/entrar-convite?codigo=${codigo}`,
        ],
      )
    }
  }
