...
export async function createComentario(req: Request, res: Response) {
  ...
  if (comentario_pai_id) {
    ...
    if (autorPai && autorPai !== userId) {
      await criarNotificacao({
        ...
        titulo: 'Nova resposta ao teu comentário',
        mensagem: 'Alguém respondeu ao teu comentário.',
        link: `/explorar/${conteudoId}`,
      })
    }
  }
  ...
  if (!comentario_pai_id) {
    ...
    if (criador && criador !== userId) {
      await criarNotificacao({
        usuarioId: criador,
        tipo: 'novo_comentario_conteudo',
        entidadeId: conteudoId,
        titulo: 'Novo comentário no teu conteúdo',
        mensagem: `Alguém comentou em "${titulo}".`,
        link: '/explorar',
      })
    }
  }
  ...
  if ((rows as RowDataPacket[]).length) {
    ...
    if (autorId && autorId !== userId) {
      await criarNotificacao({
        ...
        titulo: 'Gostaram do teu comentário',
        mensagem: 'O teu comentário recebeu um like.',
      })
    }
  }
