...
// ── GET /api/ranking?tipo=subscrito|professor|visitante ────────────────────────
// Sem `tipo`, mostra todos os utilizadores não-administrativos (mantém o
// comportamento anterior). Com `tipo`, filtra só por esse papel — permite ao
// frontend oferecer um filtro "Alunos" / "Professores" no ranking.
export async function getRanking(req: Request, res: Response) {
  const tiposValidos = ['visitante', 'subscrito', 'professor']
  const tipoFiltro = typeof req.query['tipo'] === 'string' && tiposValidos.includes(req.query['tipo'])
    ? req.query['tipo']
    : null

  const condicaoTipo = tipoFiltro ? 'AND u.tipo = ?' : "AND u.tipo NOT IN ('admin', 'superadmin')"
  const params = tipoFiltro ? [tipoFiltro] : []

  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT
       u.id, u.nome, u.avatar_url, u.provincia, u.tipo,
       SUM(r.pontuacao) AS pontuacao_total,
       SUM(r.total_perguntas) * 10 AS total_max_pontos,
