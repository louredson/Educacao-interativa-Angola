  ...
  if (!resposta_pai_id) {
    ...
    if (autorTopico && autorTopico !== userId) {
      await criarNotificacao({
        usuarioId: autorTopico,
        tipo: 'nova_resposta_forum',
        entidadeId: topicoId,
        titulo: 'Nova resposta no teu tópico',
        mensagem: 'Alguém respondeu ao teu tópico no fórum.',
        link: `/forum?topico=${topicoId}`,
      })
    }
  }
...
export async function denunciarTopico(req: Request, res: Response) {
  ...
  // Verifica se o tópico existe e obtém o título
  const [rows] = await pool.query<RowDataPacket[]>(
    'SELECT id, titulo FROM topico_forum WHERE id = ? LIMIT 1',
    [topicoId],
  )
  if (!(rows as RowDataPacket[]).length) {
    return res.status(404).json({ message: 'Tópico não encontrado.' })
  }
  const topicoTitulo: string = (rows as RowDataPacket[])[0]?.['titulo'] ?? 'Tópico'
  ...
  const [ins] = await pool.query<ResultSetHeader>(
    `INSERT IGNORE INTO denuncia (topico_forum_id, denunciado_por, motivo, descricao_detalhada)
     VALUES (?, ?, ?, ?)`,
    [topicoId, req.user!.userId, motivo, descricao_detalhada],
  )

  // Notifica todos os admins se a denúncia foi realmente inserida (não era duplicada)
  if ((ins as ResultSetHeader).affectedRows > 0) {
    await notificarAdmins({
      tipo: 'denuncia_topico',
      entidadeId: topicoId,
      titulo: 'Tópico denunciado',
      mensagem: `Um tópico foi denunciado: "${topicoTitulo.slice(0, 80)}"`,
      link: '/admin?tab=denuncias',
    })
  }
