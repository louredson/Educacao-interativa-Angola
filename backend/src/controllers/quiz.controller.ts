...
export async function gerarQuiz(req: Request, res: Response) {
  ...
  // Só admin, superadmin ou utilizadores com pode_criar_quiz podem gerar
  const podeCriar = await verificarPermissaoCriarQuiz(res, req.user!.userId, req.user!.role)
  if (!podeCriar) return
...
export async function submitAttempt(req: Request, res: Response) {
  ...
  const [ins] = await pool.query<ResultSetHeader>(
    `INSERT INTO resposta_quiz_usuario (usuario_id, quiz_id, pontuacao, total_perguntas, percentual_acerto)
     ...
     ON DUPLICATE KEY UPDATE
       pontuacao         = IF(VALUES(pontuacao) > pontuacao, VALUES(pontuacao), pontuacao),
       total_perguntas   = IF(VALUES(pontuacao) > pontuacao, VALUES(total_perguntas), total_perguntas),
       percentual_acerto = IF(VALUES(pontuacao) > pontuacao, VALUES(percentual_acerto), percentual_acerto),
       data_realizacao   = IF(VALUES(pontuacao) > pontuacao, NOW(), data_realizacao)`,
    [userId, quizId, pontos, total, percentual],
  )
  ...
  return res.status(201).json({ tentativaId: ins.insertId, total, acertos, pontos, percentual, resultados })
}
