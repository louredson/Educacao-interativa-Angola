...
export async function gerarQuizComIA(input: GerarQuizInput): Promise<QuizGerado> {
  ...
  const publico     = input.publico?.trim() || 'público geral angolano, de todas as idades e níveis de escolaridade'
  ...
  const userPrompt =
    ...
    'Adapta obrigatoriamente o vocabulário, a profundidade conceptual e a complexidade ' +
    'das perguntas e das explicações ao público-alvo indicado: linguagem simples e exemplos ' +
    'do quotidiano para públicos gerais ou escolares; termos técnicos, dados concretos e ' +
    'nuances analíticas para públicos universitários, profissionais ou académicos.\n' +
