...
export default function Resources() {
  ...
  const [resultadoFinal, setResultadoFinal] = useState<{
    total: number; acertos: number; percentual: number;
    resultados: { pergunta_id: number; resposta_escolhida: number; resposta_correta: number; correta: boolean }[];
  } | null>(null);
    ...
    const carregarRanking = async () => {
      try {
        ...
        const mapped = (rankingData ?? []).map((r: any) => ({
          ...
          totalMaxPontos: Number(r.total_max_pontos ?? 0),
        ...
        setRanking(mapped);
        if (user) {
          const pos = mapped.findIndex((r: any) => r.id === Number(user.id));
          if (pos >= 0) {
      ...
      if (currentQuestion + 1 < apiQuizData.perguntas.length) {
        ...
        try {
          const resultado = await apiRequest<{
            total: number; acertos: number; percentual: number;
            resultados: { pergunta_id: number; resposta_escolhida: number; resposta_correta: number; correta: boolean }[];
          }>(`/quizzes/${activeApiQuizId}/attempt`, {
            method: 'POST',
          ...
          if (err?.status === 429) {
            toast.info('Erro ao submeter quiz. Tenta novamente.');
          }
        }
        setShowResult(true);
        playSound('complete');
  ...
  if (showResult && activeApiQuizId && apiQuizData) {
    const totalPerguntas = apiQuizData.perguntas.length;
    // Usa sempre os dados reais devolvidos pelo backend (resultadoFinal) — o
    // contador "score" do lado do cliente não reflectia os acertos reais.
    const acertosReais    = resultadoFinal?.acertos ?? 0;
    const percentage      = resultadoFinal?.percentual ?? 0;
    const errosReais      = (resultadoFinal?.total ?? totalPerguntas) - acertosReais;

    const opcaoTexto = (pergunta: any, valor: number): string => {
      const mapa: Record<number, string> = { 1: pergunta.opcao_a, 2: pergunta.opcao_b, 3: pergunta.opcao_c, 4: pergunta.opcao_d };
      return mapa[valor] ?? '—';
    };
        ...
        <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 mb-10">
            <Card className="text-center p-6">
              <div className="text-4xl font-bold text-green-600 mb-1">{acertosReais}</div>
              <div className="text-sm text-slate-500">Acertaste</div>
            </Card>
            <Card className="text-center p-6">
              <div className="text-4xl font-bold text-red-600 mb-1">{errosReais}</div>
              <div className="text-sm text-slate-500">Erraste</div>
            </Card>
            <Card className="text-center p-6">
              <div className="text-4xl font-bold text-[#800020] mb-1">{percentage}%</div>
          ...
          {/* Revisão pergunta a pergunta — mostra exactamente o que acertou e errou */}
          {resultadoFinal && (
            <div className="mb-10">
              <h3 className="text-xl font-bold text-slate-800 mb-4">Revisão das Respostas</h3>
              <div className="space-y-3">
                {apiQuizData.perguntas.map((pergunta: any, idx: number) => {
                  const r = resultadoFinal.resultados.find(x => x.pergunta_id === pergunta.id);
                  if (!r) return null;
                  return (
                    <Card key={pergunta.id} className={`p-4 border-l-4 ${r.correta ? 'border-l-green-500' : 'border-l-red-500'}`}>
                      <div className="flex items-start gap-3">
                        <div className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-white text-sm font-bold ${r.correta ? 'bg-green-500' : 'bg-red-500'}`}>
                          {r.correta ? '✓' : '✗'}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-slate-800 text-sm mb-2">{idx + 1}. {pergunta.pergunta}</p>
                          <p className={`text-sm ${r.correta ? 'text-green-700' : 'text-red-700'}`}>
                            A tua resposta: <span className="font-semibold">{opcaoTexto(pergunta, r.resposta_escolhida)}</span>
                          </p>
                          {!r.correta && (
                            <p className="text-sm text-green-700 mt-0.5">
                              Resposta certa: <span className="font-semibold">{opcaoTexto(pergunta, r.resposta_correta)}</span>
                            </p>
                          )}
                          {pergunta.explicacao && (
                            <p className="text-xs text-slate-500 mt-2 italic">{pergunta.explicacao}</p>
                          )}
                        </div>
                      </div>
                    </Card>
          ...
          {isAuthenticated && (
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                ...
                <div className="text-sm text-[#FEE8E8]">Pontos Totais</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                <div className="text-3xl font-bold">{totalQuizzes}</div>
                <div className="text-sm text-[#FEE8E8]">Quizzes Completos</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                <div className="text-3xl font-bold">
                  {(() => {
                    const userEntry = ranking.find((r: any) => (r as any).id === Number(user?.id));
                    if (!userEntry) return 'Iniciante 🌱';
                    const totalMaxPontos = (userEntry as any).totalMaxPontos ?? 0;
                    const aproveitamento = totalMaxPontos > 0 ? (totalScore / totalMaxPontos) * 100 : 0;
                    if (aproveitamento >= 80 && totalQuizzes >= 5) return 'Avançado 🏆';
                    if (aproveitamento >= 50 && totalQuizzes >= 3) return 'Intermédio ⚔️';
                    return 'Iniciante 🌱';
                  })()}
                </div>
                <div className="text-sm text-[#FEE8E8]">Classificação</div>
              </div>
            </div>
          )}
          <div className="mt-6">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-white">Ranking de Participantes</span>
              </div>
              <button
                onClick={() => {
                  if (!isAuthenticated) {
                ...
                className="bg-white/20 hover:bg-white/30 text-white rounded-lg transition-colors text-sm flex items-center gap-2 px-3 py-1.5"
              >
            ...
            {showRanking && (
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4">
                <div className="flex flex-wrap gap-3 mb-4">
                  ...
                  >
                    {provinces.map((prov) => <option key={prov} value={prov} className="text-slate-900">{prov}</option>)}
                  </select>
                  <select
                    value={selectedTipo}
                  ...
                  >
                    ...
                    <option value="subscrito" className="text-slate-900">Alunos</option>
                    <option value="professor" className="text-slate-900">Professores</option>
                  </select>
                </div>
                {ranking.length === 0 ? (
                  <p className="text-[#FEE8E8] text-sm text-center py-4">Ainda não há participantes no ranking.</p>
                ) : (
                  <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                    {ranking
                      .filter((p: any) =>
                        (selectedProvince === 'Todas' || p.province === selectedProvince) &&
                        (selectedTipo === 'Todos' || p.tipo === selectedTipo)
                      )
                      .map((participant: any, index) => {
                        const isCurrentUser = user && participant.name === ((user as any).nome ?? user.name);
                        return (
                          <div
                            key={index}
                            className={`flex items-center justify-between p-2 rounded-lg ${isCurrentUser ? 'bg-blue-500/40' : 'bg-white/10'}`}
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-sm font-medium text-white">
                                {participant.name}
                                {isCurrentUser && <span className="ml-1 text-xs bg-blue-500 px-1 rounded">Você</span>}
                              </span>
                            </div>
                            <span className="text-sm font-bold text-yellow-300">{participant.score} pts</span>
                          </div>
                        );
                      })}
                  </div>
