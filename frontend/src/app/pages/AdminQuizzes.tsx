...
import {
  FileQuestion, PlusCircle, Trash2, Edit,
  Shield, LogOut,
...
export default function AdminQuizzes({ embedded = false }: { embedded?: boolean } = {}) {
  const { user, isAuthenticated, logout } = useAuth()
  const navigate = useNavigate()
  const formRef  = useRef<HTMLDivElement>(null)
  const [searchParams, setSearchParams] = useSearchParams()
  const editarParamConsumido = useRef(false)
  ...
  useEffect(() => {
    if (!isAuthenticated || (!user?.isAdmin && !user?.pode_criar_quiz)) { navigate('/'); return }
    void carregarTudo()
  ...
  // Página autónoma aberta com ?editar=<id> (vindo da lista do dashboard):
  // abre diretamente o formulário de edição desse quiz.
  useEffect(() => {
    if (embedded || editarParamConsumido.current || loading) return
    const editarId = Number(searchParams.get('editar'))
    if (!editarId) return
    const quiz = todosQuizzes.find(q => q.id === editarId)
    if (!quiz) return
    editarParamConsumido.current = true
    setSearchParams(prev => { prev.delete('editar'); return prev }, { replace: true })
    void handleEditar(quiz)
  }, [embedded, loading, todosQuizzes, searchParams])

  if (!user?.isAdmin && !user?.pode_criar_quiz) return null
  ...
  return (
    <div className={embedded ? '' : 'min-h-screen bg-gradient-to-b from-slate-50 to-white'}>
      ...
      {/* Topbar e header só na página autónoma /admin/quizzes — quando
          embutido no dashboard, o dashboard já tem a sua própria moldura */}
      {!embedded && (
        <>
          {/* ── Topbar ────────────────────────────────────────────────────── */}
          <nav className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex h-14 items-center justify-between">
                <Link to={user.isAdmin ? '/admin' : '/resources'} className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <section className="bg-gradient-to-r from-[#800020] via-black to-yellow-600 text-white">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
              ...
              <Link to={user.isAdmin ? '/admin' : '/resources'} className="inline-flex items-center gap-1.5 mt-3 text-sm text-white/70 hover:text-white transition-colors">
                <ArrowLeft className="w-4 h-4" /> {user.isAdmin ? 'Voltar ao Dashboard' : 'Voltar aos Quizzes'}
              </Link>
            </div>
          </section>
        </>
      ...
      <div className={embedded ? 'space-y-8' : 'max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8'}>

        {/* ── PONTO 4 — Painel Gerar com IA (só na página autónoma) ─────────── */}
        {!embedded && (
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-4 flex-wrap pb-3">
            ...
            <Button
              onClick={() => embedded ? navigate('/admin/quizzes') : resetFormulario()}
              className="bg-[#800020] hover:bg-[#5C0016] shrink-0" size="sm">
                      ...
                      {podeEditar(quiz) ? (
                        <>
                          <Button size="sm" variant="outline"
                            onClick={() => embedded ? navigate(`/admin/quizzes?editar=${quiz.id}`) : handleEditar(quiz)}
                            className="gap-1">
        ...
        {/* ── Formulário criar / editar (só na página autónoma) ──────────────── */}
        {!embedded && (
        <div ref={formRef}>
          <Card id="quiz-form">
            <CardHeader>
