...
export default function SalasDiscussao() {
  ...
  useEffect(() => {
    ...
    if (!codigoUrl || !token) return
    // Remove o parâmetro do URL imediatamente para evitar re-trigger
    setSearchParams(prev => { prev.delete('codigo'); return prev }, { replace: true })
    setAUsarCodigo(true)
    apiRequest<{ message: string; tipo: string; entidade_id: number }>('/convites/usar', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ codigo: codigoUrl.trim().toUpperCase() }),
    })
      .then(async (data) => {
        alert(data.message)
        await carregarSalas()
      ...
      .catch((err: any) => {
        alert(err?.message ?? 'Código de convite inválido ou expirado.')
      })
      .finally(() => setAUsarCodigo(false))
  }, [searchParams, token])
