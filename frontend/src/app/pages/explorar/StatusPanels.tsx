  ...
  return (
    ...
    >
      ...
      <div className={`${compact ? "mt-1" : "mt-2"} space-y-1 text-[11px] font-medium opacity-90`}>
        {isApproved ? (
          <p>
            Boa notícia! O teu pedido foi aprovado
            {accessInfo.reviewedAt ? ` a ${formatDateTime(accessInfo.reviewedAt)}` : ""} — já
            podes abrir e ler este conteúdo sempre que quiseres.
          </p>
        ) : isRejected ? (
          <p>
            Desta vez o autor não aprovou o teu pedido. Não desanimes — podes
            voltar a solicitar o acesso quando quiseres.
          </p>
        ) : (
          <p>
            O teu pedido foi enviado a {formatDateTime(accessInfo.requestedAt)} e o autor
            já foi avisado. Assim que ele responder, recebes uma notificação — não
            precisas de fazer mais nada.
          </p>
        )}
        {accessInfo.notes && <p className="italic">Mensagem do autor: “{accessInfo.notes}”</p>}
      </div>
    </div>
  );
}
